...
do
	...
	function section.new(page, title, helpcallback)
		...
		local textlbl = container.Container.Title
		if helpcallback then
			textlbl.Text = title .. "   [?]",
			textlbl.MouseButton1Click:Connect(function()
				helpcallback()
			end)
		else
			textlbl.Text = title
	...
	function library:Notify(title, text, callback, timeout)
		...
		text = text or "No information provided."
